...
let size = String.split_on_char ',' (input_line (open_in "/static/resources/level1/size.txt"))
...
let update _dt el =
  ...
  let xr = if p.x <  ((800.0-.(float_of_int b.width))/.2.0) then 0 
           else begin if p.x > ((float_of_string (List.nth size 0))-.((800.0+.(float_of_int b.width))/.2.0))
                      then ((int_of_string (List.nth size 0)) - 800)
                      else ((int_of_float (p.x+.(float_of_int b.width)/.2.0)) - 400)
                end in
  let yr = if p.y < ((600.0-.(float_of_int b.height))/.2.0) then 0
           else begin if p.y > ((float_of_string (List.nth size 1))-.((600.0+.(float_of_int b.height))/.2.0))
                      then ((int_of_string (List.nth size 1)) - 600)
